...
class Lexer:
    ...
    def _tokenize_operator(self) -> None:
        ...
        if ch == "%":
            # Если предыдущий токен - число или закрывающая скобка, И следующий токен не число
            # то это унарный процент (например: 50%, (100+50)%)
            if self.tokens and self.tokens[-1].type in (TokenType.NUMBER, TokenType.RPAREN):
                ...
                # Например: 17%5 - после % идет число 5
                next_pos = self.pos + 1
